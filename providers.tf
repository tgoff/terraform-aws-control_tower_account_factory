...
provider "aws" {
  ...
  region = var.ct_home_region
  assume_role {
    role_arn     = "arn:${data.aws_partition.current.partition}:iam::${var.audit_account_id}:role/AWSAFTExecution"
    session_name = local.aft_session_name
  }
  default_tags {
    tags = {
...
provider "aws" {
  ...
  region = var.ct_home_region
  assume_role {
    role_arn     = "arn:${data.aws_partition.current.partition}:iam::${var.log_archive_account_id}:role/AWSAFTExecution"
    session_name = local.aft_session_name
  }
  default_tags {
    tags = {
